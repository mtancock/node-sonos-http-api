...
const musicTypes = ['album','song','station','load','playlist',
                    'playsongfromhash','playalbumfromhash',
                    'queuesongfromhash','queuealbumfromhash'];
...
function doSearch(service, type, term)
{
  ...
  if (!type.endsWith('hash') && term.indexOf(':') > -1) {
    var newTerm = '';
          ...
          if (tracks.count == 0) {
            ...
            if (tracks.isArtist) {  // Play numerous songs by the specified artist
              ...
              if (type.endsWith('hash')) {
                // TODO: Add support for queuealbumfromhash/playalbumfromhash
                let track = tracks.queueTracks[0];
                console.log(type + " " + track);
                let uri = tracks.queueTracks[0].uri;
                let metadata = tracks.queueTracks[0].metadata;
