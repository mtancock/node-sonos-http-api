...
function loadFuseForHash(items, fuzzyKeys) {
  return new Promise((resolve) => {
    return resolve(new Fuse(items, { keys: fuzzyKeys, caseSensitive: true, shouldSort: true, threshold: 0.0, distance: 0, maxPatternLength: 40 }));
  });
}
  ...
  let getChunk = (chunk) => {
    chunk.items.reduce(function (tracksArray, item) {
      if ((item.uri != undefined) && (item.artist != undefined) && (item.album != undefined)) {
        ...
        //logger.info('Song: ' + item.title + ' by ' + item.artist + ' on ' + item.album + ' uri ' + item.uri + ' hash ' + md5hash);
        result.items.push({
...
function loadLibrarySearch(player, load) {
  if (load || (musicLibrary == null)) {
    return loadLibrary(player)
      ...
      .then(() => loadFuseForHash(musicLibrary.tracks.items, ["md5uri"]))
      .then((result) => {
        fuzzyHashes = result;
      })
...
function searchLibrary(type, term) {
  ...
  // TODO: For playalbumfromhash and queuealbumfromhash, we need to return the full album associated with the song
  if (type.endsWith('hash')) {
    console.log("Searching for hash: " + term);
    return fuzzyHashes.search(term).slice(0, 1);
  }
